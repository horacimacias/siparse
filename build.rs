...
fn main() {
    ...
    let version = String::from_utf8(output).expect("rustc version output should be utf-8");
...
fn enable_new_features(raw_version: &str) {
    ...
    if version >= min_rust2018_version {
        println!("cargo:rustc-cfg=siparse_min_2018");
    }
...
fn enable_simd(version: Version) {
    if env::var_os("CARGO_FEATURE_STD").is_none() {
        println!("cargo:warning=building for no_std disables siparse SIMD");
        return;
    }

    let env_disable = "CARGO_CFG_SIPARSE_DISABLE_SIMD";
    ...
    if version >= min_simd_version {
        println!("cargo:rustc-cfg=siparse_simd");
    }
    ...
    let env_runtime_only = "CARGO_CFG_SIPARSE_DISABLE_SIMD_COMPILETIME";
    ...
    for feature in feature_list.split(',') {
        ...
        if !saw_sse42 && feature == "sse4.2" {
            saw_sse42 = true;
            println!("cargo:rustc-cfg=siparse_simd_target_feature_sse42");
        }

        if !saw_avx2 && feature == "avx2" {
            saw_avx2 = true;
            println!("cargo:rustc-cfg=siparse_simd_target_feature_avx2");
        }
    }
}
